...
class Conversation(UI):
    ...
    def get_next_target(self):
        ...
        if _:
            ...
            # 咨询状态
            if CASE_CLOSED.match(
                crop(self.device.image, check_area), threshold=0.71, static=False
            ):
        ...
        if not self.appear(CONVERSATION_CHECK, offset=(10, 10)):
            logger.warning(
                "Perhaps the current page was changed by a human or certain error"
            )
    ...
    def ensure_in_detail(self, skip_first_screenshot=True):
        ...
        while 1:
            ...
            if click_timer.reached() and not DETAIL_CHECK.match(self.device.image):
        ...
        if RANK_MAX_CHECK.match(self.device.image):
            ...
            while 1:
                ...
                if click_timer.reached() and not CONVERSATION_CHECK.match(
                    self.device.image
                ):
                ...
                if (
                    ...
                    self.device.image = mask_area(
                        self.device.image, FAVOURITE_CHECK._button_offset
                    )
    ...
    def ensure_wait_to_answer(self, skip_first_screenshot=True):
        ...
        while 1:
            ...
            if click_timer.reached() and self.appear_then_click(
                CONFRIM_B, offset=(30, 30), interval=3
            ):
                click_timer.reset()
                continue
    ...
    def answer(self, skip_first_screenshot=True):
        ...
        answer_a = self.ocr(
            extract_letters(
                crop(self.device.image, answer_a_area), letter=(247, 243, 247)
            ),
            label="OCR_ANSWER_A",
        )
        answer_b = self.ocr(
            extract_letters(
                crop(self.device.image, answer_b_area), letter=(247, 243, 247)
            ),
            label="OCR_ANSWER_B",
        )
        ...
        try:
            if answer_a is not None and answer_b is not None:
                ...
                if ratio_a > ratio_b >= 0:
                    logger.info(
                        "Answer A seems to be the correct one similar to %s", similar_a
                    )
                    self.device.click_minitouch(*find_center(answer_a_area))
                elif ratio_b > ratio_a >= 0:
                    logger.info(
                        "Answer B seems to be the correct one similar to %s", similar_b
                    )
        ...
        while 1:
            ...
            if self.appear(DETAIL_CHECK, offset=(30, 30), static=False) or self.appear(
                RANK_INCREASE_CHECK, offset=(30, 30), static=False
            ):
                break
